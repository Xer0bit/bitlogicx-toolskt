  ...
  const handleConvert = async (event) => {
    ...
    try {
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_API_URL}/tools/jpg-to-png/`,
        formData,
        {
          headers: {
      ...
      setConvertedFile(`data:image/png;base64,${response.data}`);
    } catch (error) {
    ...
    <div className="max-w-6xl mx-auto py-12 px-4 sm:py-16 sm:px-6 lg:px-8">
      <div className="text-left">
        <div className="text-3xl flex gap-3 items-center font-extrabold tracking-tight text-indigo-500 sm:text-4xl">
          <AiOutlineFileJpg />
          <h2 className="text-gray-900">JPG To PNG Converter</h2>
        </div>
        <p className="mt-3 max-w-md text-base text-gray-500 sm:mt-5 sm:text-lg sm:max-w-xl">
          Easily convert Jpg images to Png with our powerful tool.
        </p>
      </div>
      <div className="bg-background rounded-lg border p-6 w-full mt-5">
        <form className="space-y-4" onSubmit={handleConvert}>
          ...
          <button
            ...
            disabled={loading}
          >
            {loading ? "Processing..." : "Convert to PNG"}
          </button>
        </form>
        {convertedFile && (
        ...
        {!convertedFile && (
          <p className="mt-4 text-sm text-muted-foreground">
            The converted PNG file will be available for download once the
