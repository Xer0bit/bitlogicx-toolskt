  ...
  const handleConvert = async (event) => {
    ...
    try {
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_API_URL}/tools/split-pdfs/`,
        formData,
        {
          headers: {
    ...
    <div className="max-w-6xl mx-auto py-12 px-4 sm:py-16 sm:px-6 lg:px-8">
      <div className="text-left">
        <div className="text-3xl flex gap-3 items-center font-extrabold tracking-tight text-indigo-500 sm:text-4xl">
          ...
          <h2 className="text-gray-900">Split PDFs</h2>
        </div>
        <p className="mt-3 max-w-md text-base text-gray-500 sm:mt-5 sm:text-lg sm:max-w-xl">
          Split PDF files into multiple parts based on the page range specified.
        </p>
      </div>
      <div className="bg-background rounded-lg border p-6 w-full mt-5">
        <form className="space-y-4" onSubmit={handleConvert}>
          ...
          <div>
            ...
            <input
              id="page_range"
              type="text"
              className="block w-full rounded-md border border-input bg-transparent px-3 py-2 text-sm placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2"
            />
          </div>
          <button
            type="submit"
        ...
        {resultFile && (
          <div className="mt-4">
            <div className="flex items-center border w-fit p-1 gap-1 rounded-md">
              ...
              <span className="opacity-70 text-sm">split-documents</span>
            </div>
            <a
              href={resultFile}
              download="splits.zip"
        ...
        {!resultFile && (
          <p className="mt-4 text-sm text-muted-foreground">
            The split files will be available for download once the operation is
            complete.
          </p>
        )}
      </div>
