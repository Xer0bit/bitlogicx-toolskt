  ...
  const handleConvert = async (event) => {
    ...
    try {
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_API_URL}/tools/text-to-qr/`,
        { text },
        {
          headers: {
      ...
      <div className="max-w-6xl mx-auto py-12 px-4 sm:py-16 sm:px-6 lg:px-8">
        <div className="text-left">
          <div className="text-3xl flex gap-3 items-center font-extrabold tracking-tight text-indigo-500 sm:text-4xl">
            <IoQrCodeOutline />
            <h2 className="text-gray-900">Text To QR Code Generator</h2>
          </div>
          <p className="mt-3 max-w-md text-base text-gray-500 sm:mt-5 sm:text-lg sm:max-w-xl">
            Easily generate a QR code from your text.
          </p>
        </div>
        <div className="bg-background rounded-lg border p-6 w-full mt-5">
          ...
          {qrCodeImage && (
            <div className="mt-4">
              <img src={qrCodeImage} alt="QR Code" className="w-1/5" />
              <a
                href={qrCodeImage}
          ...
          {!qrCodeImage && (
            <p className="mt-4 text-sm text-muted-foreground">
              The generated QR code will be available for download once the
