...
export default function AsciiToBinaryConverter() {
  ...
  const convertAsciiToBinary = () => {
    ...
    for (let i = 0; i < asciiStr.length; i++) {
      ...
      binary += binaryChar + " "; // Add a space between each byte for readability
    }

    setBinaryResult(binary.trim()); // Trim trailing space
  };
      ...
      <div className="flex flex-col gap-10">
        <div className="text-left">
          <div className="text-3xl flex gap-3 items-center font-extrabold tracking-tight text-indigo-500 sm:text-4xl">
            ...
            <h2 className="text-gray-900">ASCII to Binary Converter</h2>
          </div>
          <p className="mt-3 max-w-md text-base text-gray-500 sm:mt-5 sm:text-lg sm:max-w-xl">
            Convert ASCII text to its binary representation using the input
            field below.
          </p>
        </div>
        <div className="flex flex-col lg:flex-row gap-6">
          <div className="w-full">
            <label className="block mb-1 text-sm font-medium">ASCII Text</label>
            <input
              type="text"
              value={ascii}
