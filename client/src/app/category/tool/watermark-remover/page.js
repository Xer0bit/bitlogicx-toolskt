  ...
  const handleRemoveWatermark = async () => {
    ...
    try {
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_API_URL}/tools/watermark-remover/`,
        formData
      );

      setEditedImage(`data:image/png;base64,${response.data.image}`);
    } catch (error) {
      ...
      <div className="flex flex-col">
        <div className="text-left">
          <div className="text-3xl flex gap-3 items-center font-extrabold tracking-tight text-indigo-500 sm:text-4xl">
            <IoImagesOutline />
            <h2 className="text-gray-900">Watermark Remover</h2>
          </div>
          <p className="mt-3 max-w-md text-base text-gray-500 sm:mt-5 sm:text-lg sm:max-w-xl">
            Easily remove watermarks from your images with our powerful tool.
          </p>
        </div>
        <div className="mt-12">
