...
export default function BinaryToHexadecimalConverter() {
  ...
  const convertBinaryToHex = () => {
    if (!binary.match(/^[01\s]+$/)) {
      setHexResult("Invalid binary input");
      return;
    }

    const binaryStr = binary.replace(/\s+/g, ""); // Remove any spaces
    const hex = parseInt(binaryStr, 2).toString(16).toUpperCase();
    setHexResult(hex);
  };
      ...
      <div className="flex flex-col gap-10">
        <div className="text-left">
          <div className="text-3xl flex gap-3 items-center font-extrabold tracking-tight text-indigo-500 sm:text-4xl">
            ...
            <h2 className="text-gray-900">Binary to Hexadecimal Converter</h2>
          </div>
          <p className="mt-3 max-w-md text-base text-gray-500 sm:mt-5 sm:text-lg sm:max-w-xl">
            Convert binary representation to hexadecimal using the input field
            below.
          </p>
        </div>
